@foreach (App\Services\TranslatableService::getTranslatableInputs(App\Models\Slider::class) as $name => $data)
...
@endforeach
{{-- <div class="col-md-6">
    <label for="link" class="form-label">{{ translate('link') }}</label>
    <input type="url" class="form-control" name="link" value="{{ old('link', $row['link'] ?? '') }}" id="link">
    @error('link')
        <div class="invalid-feedback d-block">{{ $message }}</div>
    @enderror
</div> --}}
